  ...
  return <DashboardShell>
    {!profile && !error ? <Skeleton className="h-64 w-full" /> : <>
      ...
      <div className="grid grid-cols-1 md:grid-cols-[1.4fr_1fr] gap-4 md:gap-6 mb-10">
        ...
        <div className="bg-slate border border-line px-7 py-7 flex flex-col justify-between"><div><div className="text-[14px] text-paper-dim mb-2">Available cash</div><div className="font-mono text-[26px]">{formatCurrency(profile?.available_cash ?? 0)}</div></div><div className="flex gap-3 mt-6"><Link href="/dashboard/deposit" className="flex-1 text-center bg-gold text-ink text-[13.5px] font-medium py-2.5">Deposit</Link><Link href="/dashboard/withdraw" className="flex-1 text-center border border-line text-paper text-[13.5px] py-2.5">Withdraw</Link></div></div>
      </div>
